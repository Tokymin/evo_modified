...
def parser() -> argparse.ArgumentParser:
    ...
    kitti_parser = sub_parsers.add_parser(
        "kitti", description="%s for KITTI pose source - %s" %
                             (basic_desc, lic), parents=[shared_parser])
    kitti_parser.add_argument("pose_files", help="one or multiple pose source",
                              nargs='+')

    tum_parser = sub_parsers.add_parser(
        "tum", description="%s for TUM trajectory source - %s" %
                           (basic_desc, lic), parents=[shared_parser])
    tum_parser.add_argument("traj_files",
                            help="one or multiple trajectory source", nargs='+')
    ...
    bag_parser = sub_parsers.add_parser(
        "bag", description="%s for ROS bag source - %s" % (basic_desc, lic),
