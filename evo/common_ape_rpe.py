...
def plot_result(args: argparse.Namespace, result: Result, traj_ref: PosePath3D,
    ...
    if args.save_plot:
        if not os.path.exists(args.save_plot + model_prefix):
            os.makedirs(args.save_plot + model_prefix)
        plot_collection.export(args.save_plot + model_prefix + "/" + str(test_dataset_index),
